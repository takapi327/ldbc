...
object UpdateTest extends ZIOSpecDefault:
      ...
      datasource.use { conn =>
        for
          ...
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_boolean_table`(`c1` BOOLEAN NOT NULL, `c2` BOOLEAN NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_boolean_table`(`c1`, `c2`) VALUES (?, ?)")
          count <- preparedStatement.setBoolean(1, true) *> preparedStatement
                     .setNull(2, MysqlType.BOOLEAN.jdbcType) *> preparedStatement
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_boolean_table`")
      ...
      datasource.use { conn =>
        for
          ...
          count <- preparedStatement.setByte(1, 1.toByte) *> preparedStatement
                     ...
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_byte_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("Short values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_short_table`(`c1` TINYINT NOT NULL, `c2` TINYINT NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_short_table`(`c1`, `c2`) VALUES (?, ?)")
          count <- preparedStatement.setShort(1, 1.toShort) *> preparedStatement
                     .setNull(2, MysqlType.TINYINT.jdbcType) *> preparedStatement
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_short_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("Int values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_int_table`(`c1` SMALLINT NOT NULL, `c2` SMALLINT NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_int_table`(`c1`, `c2`) VALUES (?, ?)")
          count <-
            preparedStatement.setInt(1, 1) *> preparedStatement.setNull(
              2,
              MysqlType.SMALLINT.jdbcType
            ) *> preparedStatement.executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_int_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("Long values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_long_table`(`c1` BIGINT NOT NULL, `c2` BIGINT NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_long_table`(`c1`, `c2`) VALUES (?, ?)")
          count <-
            preparedStatement.setLong(1, Long.MaxValue) *> preparedStatement
              .setNull(2, MysqlType.BIGINT.jdbcType) *> preparedStatement
              .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_long_table`")
        yield assert(count)(equalTo(1))
      }
    },
        ...
        for
          ...
          _         <-
            statement.executeUpdate(
              "CREATE TABLE `zio_client_statement_bigint_table`(`c1` BIGINT unsigned NOT NULL, `c2` BIGINT unsigned NULL)"
            )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_bigint_table`(`c1`, `c2`) VALUES (?, ?)")
          count <- preparedStatement.setString(1, "18446744073709551615") *> preparedStatement
                     .setNull(2, MysqlType.BIGINT.jdbcType) *> preparedStatement
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_bigint_table`")
        yield assert(count)(equalTo(1))
      }
    },
      ...
      datasource.use { conn =>
        for
          ...
          count <- preparedStatement.setFloat(1, 1.1f) *> preparedStatement
                     ...
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_float_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("Double values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_double_table`(`c1` DOUBLE NOT NULL, `c2` DOUBLE NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_double_table`(`c1`, `c2`) VALUES (?, ?)")
          count <- preparedStatement.setDouble(1, 1.1) *> preparedStatement
                     .setNull(2, MysqlType.DOUBLE.jdbcType) *> preparedStatement
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_double_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("BigDecimal values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_bigdecimal_table`(`c1` DECIMAL NOT NULL, `c2` DECIMAL NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_bigdecimal_table`(`c1`, `c2`) VALUES (?, ?)")
          count <- preparedStatement.setBigDecimal(1, BigDecimal.decimal(1.1)) *> preparedStatement
                     ...
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_bigdecimal_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("String values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_string_table`(`c1` VARCHAR(255) NOT NULL, `c2` VARCHAR(255) NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_string_table`(`c1`, `c2`) VALUES (?, ?)")
          count <- preparedStatement.setString(1, "test") *> preparedStatement
                     .setNull(2, MysqlType.VARCHAR.jdbcType) *> preparedStatement
                     .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_string_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("Array[Byte] values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_bytes_table`(`c1` BINARY(10) NOT NULL, `c2` BINARY NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_bytes_table`(`c1`, `c2`) VALUES (?, ?)")
          count <-
            preparedStatement.setBytes(1, Array[Byte](98, 105, 110, 97, 114, 121)) *> preparedStatement
              ...
              .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_bytes_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("java.time.LocalTime values can be set as parameters") {
      datasource.use { conn =>
        for
          ...
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_time_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("java.time.LocalDate values can be set as parameters") {
      datasource.use { conn =>
        for
          ...
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_date_table`")
        yield assert(count)(equalTo(1))
      }
    },
    test("java.time.LocalDateTime values can be set as parameters") {
      datasource.use { conn =>
        for
          statement <- conn.createStatement()
          _         <- statement.executeUpdate(
                 "CREATE TABLE `zio_client_statement_datetime_table`(`c1` TIMESTAMP NOT NULL, `c2` TIMESTAMP NULL)"
               )
          preparedStatement <-
            conn.prepareStatement("INSERT INTO `zio_client_statement_datetime_table`(`c1`, `c2`) VALUES (?, ?)")
          ...
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_datetime_table`")
        yield assert(count)(equalTo(1))
      }
    },
        ...
        for
          ...
          count <-
            preparedStatement.setInt(1, 2020) *> preparedStatement
              ...
              .executeUpdate()
          _ <- statement.executeUpdate("DROP TABLE `zio_client_statement_year_table`")
        yield assert(count)(equalTo(1))
      }
    }
  )
