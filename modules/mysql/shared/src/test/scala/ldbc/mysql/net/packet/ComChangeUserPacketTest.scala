...
import ldbc.mysql.net.packet.request.ComChangeUserPacket
...
class ComChangeUserPacketTest extends FTestPlatform:
  ...
  test("ComChangeUserPacket creation and properties") {
    ...
    assertEquals(
      packet.capabilitiesFlags,
      Set(CapabilitiesFlags.CLIENT_PROTOCOL_41, CapabilitiesFlags.CLIENT_PLUGIN_AUTH)
    )
  ...
  test("ComChangeUserPacket encoder with CLIENT_PROTOCOL_41 and CLIENT_PLUGIN_AUTH") {
    ...
    assert(
      encodedBytes.containsSlice(
        ByteVector.encodeAscii("mysql_native_password").getOrElse(ByteVector.empty) ++ ByteVector(0)
      )
    )
  }
  ...
  test("ComChangeUserPacket encoder with CLIENT_RESERVED2 flag") {
    ...
    val hashedPwdIndex =
      encodedBytes.indexOfSlice(ByteVector.encodeAscii("testuser").getOrElse(ByteVector.empty) ++ ByteVector(0)) +
        "testuser".length + 1
