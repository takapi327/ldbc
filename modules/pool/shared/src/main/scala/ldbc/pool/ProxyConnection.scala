 ...
 *
 * This implementation uses [[FastList]] for statement tracking to minimise allocations and maximise
 * performance, following HikariCP's approach.
 *
 * @param pooled             the pooled connection being wrapped
 * @param releaseCallback    the callback to invoke when the connection is released
 * @param closeAllStatements whether to close all statements when the connection is released
 */
private[pool] class ProxyConnection[F[_]](
  ...
  override def close(): F[Unit] =
    val cleanup = if closeAllStatements then closeTrackedStatements() else F.unit
  ...
  /**
   * Closes all tracked statements. This is called when the connection is returned to the pool to
   * ensure no statements leak.
   */
