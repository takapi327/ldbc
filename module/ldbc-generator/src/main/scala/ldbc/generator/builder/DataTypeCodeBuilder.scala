...
case class DataTypeCodeBuilder(scalaType: String, formatter: Naming):
  ...
  def build(dataType: DataType): String =
    dataType match
      ...
      case data: DataType.NumberDataType => data.length.fold(s"${ data.name }[$scalaType]")(n => s"${ data.name }[$scalaType]($n)") + buildNumberDataType(data.unsigned, data.zerofill)
      case data: DataType.DECIMAL => s"${ data.name }[$scalaType](${ data.accuracy }, ${ data.scale })" + buildNumberDataType(data.unsigned, data.zerofill)
      case data: DataType.FLOAT => s"${ data.name }[$scalaType](${ data.accuracy })" + buildNumberDataType(data.unsigned, data.zerofill)
      case data: DataType.StringDataType => s"${ data.name }[$scalaType](${ data.length })" + buildCharacterSet(data.character, data.collate)
      ...
      case data: DataType.ENUM => s"${data.name}[$scalaType](using ${ scalaType.replace("Option[", "").replace("]", "") })" + buildCharacterSet(data.character, data.collate)
      case data: DataType.BLOB => data.length.fold(s"${ data.name }[$scalaType]()")(n => s"${ data.name }[$scalaType]($n)")
      ...
      case data: DataType.MEDIUMTEXT => s"${data.name}[$scalaType]()" + buildCharacterSet(data.character, data.collate)
      ...
      case data: DataType.DATETIME => data.fsp.fold(s"${ data.name }[$scalaType]")(n => s"${ data.name }[$scalaType]($n)")
      case data: DataType.TIMESTAMP => data.fsp.fold(s"${ data.name }[$scalaType]")(n => s"${ data.name }[$scalaType]($n)")
