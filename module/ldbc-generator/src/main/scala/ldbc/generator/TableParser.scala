  ...
  private def tableOption: Parser[Table.Options] =
    keyValue(caseSensitivity("autoextend_size"), sqlIdent) ^^ Table.Options.AutoExtendSize.apply |
      ...
      keyValue(caseSensitivity("checksum"), digit) ^^ {
        ...
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the checksum; the checksum must be one of the values 0 or 1.")
      } |
      ...
      keyValue(caseSensitivity("compression"), sqlIdent) ^^ {
        ...
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the compression; the checksum must be one of the values ZLIB, LZ4 or NONE.")
      } |
      keyValue(caseSensitivity("connection"), sqlIdent) ^^ Table.Options.Connection.apply |
      keyValue(caseSensitivity("data") | caseSensitivity("index") ~> caseSensitivity("directory"), sqlIdent) ^^ Table.Options.Directory.apply |
      keyValue(caseSensitivity("delay_key_write"), digit) ^^ {
        case value: (0 | 1) => Table.Options.DelayKeyWrite(value)
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the delay_key_write; the checksum must be one of the values 0 or 1.")
      } |
      keyValue(caseSensitivity("encryption"), sqlIdent) ^^ {
        case value: ("Y" | "N") => Table.Options.Encryption(value)
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the encryption; the checksum must be one of the values Y or N.")
      } |
      ...
      keyValue(caseSensitivity("insert_method"), sqlIdent) ^^ {
        ...
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the insert_method; the checksum must be one of the values NO, FIRST or LAST.")
      } |
      ...
      keyValue(caseSensitivity("pack_keys"), sqlIdent) ^^ {
        ...
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the pack_keys; the checksum must be one of the values 0, 1 or DEFAULT.")
      } |
      keyValue(caseSensitivity("password"), sqlIdent) ^^ Table.Options.Password.apply |
      keyValue(caseSensitivity("row_format"), sqlIdent) ^^ {
        case value: ("DEFAULT" | "DYNAMIC" | "FIXED" | "COMPRESSED" | "REDUNDANT" | "COMPACT") => Table.Options.RowFormat(value)
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the pack_keys; the checksum must be one of the values DEFAULT, DYNAMIC, FIXED, COMPRESSED, REDUNDANT, or COMPACT.")
      } |
      keyValue(caseSensitivity("secondary_engine_attribute"), sqlIdent) ^^ Table.Options.SecondaryEngineAttribute.apply |
      keyValue(caseSensitivity("stats_auto_recalc"), sqlIdent) ^^ {
        case value: ("0" | "1" | "DEFAULT") => Table.Options.StatsAutoRecalc(value)
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the stats_auto_recalc; the checksum must be one of the values 0, 1 or DEFAULT.")
      } |
      keyValue(caseSensitivity("stats_persistent"), sqlIdent) ^^ {
        case value: ("0" | "1" | "DEFAULT") => Table.Options.StatsPersistent(value)
        case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the stats_persistent; the checksum must be one of the values 0, 1 or DEFAULT.")
      } |
      keyValue(caseSensitivity("stats_sample_pages"), sqlIdent) ^^ Table.Options.StatsSamplePages.apply |
      caseSensitivity("tablespace") ~> sqlIdent ~ opt(caseSensitivity("storage") ~> caseSensitivity("disk") | caseSensitivity("memory")) ^^ {
        case name ~ storage => Table.Options.Tablespace(name, storage.map {
          case "DISK" => "DISK"
          case "MEMORY" => "MEMORY"
          case unknown => throw new IllegalArgumentException(s"$unknown is not a value that can be set in the tablespace storage; the checksum must be one of the values DISK or MEMORY.")
        })
      } |
