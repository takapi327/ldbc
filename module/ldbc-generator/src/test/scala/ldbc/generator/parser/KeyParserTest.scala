...
class KeyParserTest extends AnyFlatSpec, KeyParser:
  ...
  it should "Key definitions parsing test succeeds." in {
    ...
    assert(parseAll(keyDefinitions, "constraint `index` foreign key (`column1`) references table_name (`column1`) match full on delete restrict").successful)
    assert(parseAll(keyDefinitions, "constraint foreign key (`column1`) references table_name (`column1`) on update set null").successful)
    assert(parseAll(keyDefinitions, "foreign key (`column1`) references table_name (`column1`) match full on delete restrict on update cascade").successful)
    assert(parseAll(keyDefinitions, "constraint check (column1 > 1)").successful)
    assert(parseAll(keyDefinitions, "INDEX `index` USING HASH (`column`) KEY_BLOCK_SIZE=1 USING HASH WITH PARSER ngram COMMENT 'comment' VISIBLE ENGINE_ATTRIBUTE=InnoDB SECONDARY_ENGINE_ATTRIBUTE=InnoDB").successful)
  }
  ...
  it should "Key definitions parsing test fails." in {
    ...
    assert(!parseAll(keyDefinitions, "foreign key `index` (title,body) references table_name (title,body) match failed").successful)
    assert(!parseAll(keyDefinitions, "foreign key `index` (title,body) references table_name (title,body) match FULL on delete failed").successful)
    assert(!parseAll(keyDefinitions, "foreign key `index` (title,body) references table_name (title,body) match FULL on failed failed").successful)
    assert(!parseAll(keyDefinitions, "foreign key `index` (title,body) references table_name (title,body) match FULL on failed SET NULL").successful)
