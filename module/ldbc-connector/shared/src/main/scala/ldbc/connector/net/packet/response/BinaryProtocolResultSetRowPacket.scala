...
object BinaryProtocolResultSetRowPacket:
  ...
  //  if input.toString.length == 1 then "0" + input
  //  else input.toString
  ...
  def decodeValue(column: ColumnDefinitionPacket, isColumnNull: Boolean): Decoder[Option[String]] =
    if isColumnNull then provide(None)
      ...
      column.columnType match
        case MYSQL_TYPE_STRING | MYSQL_TYPE_VARCHAR | MYSQL_TYPE_VAR_STRING | MYSQL_TYPE_ENUM | MYSQL_TYPE_SET | MYSQL_TYPE_LONG_BLOB | MYSQL_TYPE_MEDIUM_BLOB | MYSQL_TYPE_BLOB | MYSQL_TYPE_TINY_BLOB | MYSQL_TYPE_GEOMETRY | MYSQL_TYPE_BIT | MYSQL_TYPE_DECIMAL | MYSQL_TYPE_NEWDECIMAL =>
        ...
        case MYSQL_TYPE_DATE => date.map(localDateFormatter.format(_).some)
        case MYSQL_TYPE_DATETIME | MYSQL_TYPE_TIMESTAMP => timestamp.map(_.map(localDateFormatter.format(_)))
    ...
    uint8L.flatMap {
      ...
      case OKPacket.STATUS =>
        for
          ...
          values     <- columns.zipWithIndex.traverse { case (column, index) =>
