...
private[ldbc] case class DatabaseMetaDataUsingInfoSchema[F[_]: Temporal: Exchange: Tracer](
...
)(using ev: MonadError[F, Throwable]) extends Impl[F](protocol, serverVariables, database, databaseTerm, getProceduresReturnsFunctions):
  ...
  override def getProcedures(catalog: Option[String], schemaPattern: Option[String], procedureNamePattern: Option[String]): F[ResultSet[F]] =
    ...
    val sqlBuf = new StringBuilder(
      if databaseTerm.contains(DatabaseTerm.SCHEMA) then "SELECT ROUTINE_CATALOG AS PROCEDURE_CAT, ROUTINE_SCHEMA AS PROCEDURE_SCHEM,"
