...
case class LdbcResultSet[F[_]](
  ...
  override def getMetaData(): F[ResultSetMetaData] =
    checkClose {
      ev.pure(LdbcResultSetMetaData(columns, serverVariables, version))
    }
...
object LdbcResultSet:
  ...
  def apply[F[_]](
    ...
    currentCursor:          Ref[F, Int],
    currentRow:             Ref[F, Option[ResultSetRowPacket]]
  )(using MonadError[F, Throwable]): LdbcResultSet[F] =
    LdbcResultSet[F](
      columns,
      records,
      serverVariables,
