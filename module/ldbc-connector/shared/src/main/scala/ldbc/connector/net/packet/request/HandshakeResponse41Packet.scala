...
object HandshakeResponse41Packet:
  ...
  val encoder: Encoder[HandshakeResponse41Packet] = Encoder { handshakeResponse =>
    ...
    val authResponse =
      if handshakeResponse.capabilitiesFlags.contains(CapabilitiesFlags.CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA) then
        BitVector(handshakeResponse.hashedPassword)
      else BitVector(copyOf(handshakeResponse.hashedPassword, handshakeResponse.hashedPassword.length))
    ...
    val pluginName =
      if handshakeResponse.capabilitiesFlags.contains(CapabilitiesFlags.CLIENT_PLUGIN_AUTH) then
        nullTerminatedStringCodec.encode(handshakeResponse.pluginName).require
      else BitVector.empty

    val attrs =
      if handshakeResponse.capabilitiesFlags.contains(CapabilitiesFlags.CLIENT_CONNECT_ATTRS) then BitVector(0x00)
      else BitVector.empty

    val zstdCompressionLevel =
      if handshakeResponse.capabilitiesFlags.contains(CapabilitiesFlags.CLIENT_ZSTD_COMPRESSION_ALGORITHM) then
        BitVector(0x00)
      else BitVector.empty
