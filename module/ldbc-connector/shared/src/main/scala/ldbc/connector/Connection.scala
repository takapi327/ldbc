...
trait Connection[F[_]]:
   ...
   */
  def clientPreparedStatement(sql: String): F[PreparedStatementImpl.Client[F]]
  ...
  ): F[PreparedStatementImpl.Client[F]]
...
object Connection:
  ...
  private[ldbc] case class ConnectionImpl[F[_]: Temporal: Tracer: Console: Exchange](
    ...
    override def prepareStatement(sql: String, resultSetType: Int, resultSetConcurrency: Int): F[PreparedStatement[F]] =
      for
        ...
        autoGeneratedKeys <-
          Ref[F].of(Statement.NO_GENERATED_KEYS)
        lastInsertId <- Ref[F].of(0)
      yield PreparedStatementImpl.Client[F](
        protocol,
        serverVariables,
        sql,
        params,
    ...
    override def clientPreparedStatement(sql: String): F[PreparedStatementImpl.Client[F]] =
