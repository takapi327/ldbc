  ...
  private[ldbc] case class ConnectionImpl[F[_]: Temporal: Tracer: Console: Exchange](
    ...
    override def prepareCall(sql: String, resultSetType: Int, resultSetConcurrency: Int): F[CallableStatement[F]] =
      for
        ...
        parameterIndexToRsIndex <- Ref[F].of(List.fill(paramInfo.numParameters)(CallableStatement.NOT_OUTPUT_PARAMETER_INDICATOR).zipWithIndex.map((param, index) => index -> param).toMap)
        updateCount      <- Ref[F].of(-1)
        moreResults      <- Ref[F].of(false)
        autoGeneratedKeys <-
          Ref[F].of[Statement.NO_GENERATED_KEYS | Statement.RETURN_GENERATED_KEYS](Statement.NO_GENERATED_KEYS)
    ...
    override def prepareStatement(
      sql:               String,
      autoGeneratedKeys: NO_GENERATED_KEYS | RETURN_GENERATED_KEYS
    ): F[PreparedStatement[F]] =
      for
        params           <- Ref[F].of(ListMap.empty[Int, Parameter])
        batchedArgs      <- Ref[F].of(Vector.empty[String])
