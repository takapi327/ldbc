...
object PoolConfigValidator:
  ...
  private[pool] def validateMinConnections[F[_]: Async](config: MySQLConfig): F[Unit] =
    if config.minConnections < 0 then
      Async[F].raiseError(new IllegalArgumentException(s"minConnections cannot be less than 0, value: ${config.minConnections}"))
    else
      Async[F].unit

  private[pool] def validateMaxConnections[F[_]: Async: Console](config: MySQLConfig): F[Unit] =
    if config.maxConnections <= 0 then
      Async[F].raiseError(new IllegalArgumentException(s"maxConnections cannot be less than 1, value: ${config.maxConnections}"))
    else if config.maxConnections > 100 && config.debug then
      Console[F].println(s"${AnsiColor.YELLOW} [WARN] ${AnsiColor.RESET} maxConnections (${config.maxConnections}) is unusually high, consider reducing it for better performance.")
    else
      Async[F].unit
  ...
  private[pool] def validateConnectionTimeout[F[_]: Async](config: MySQLConfig): F[Unit] =
    if config.connectionTimeout <= Duration.Zero then
      Async[F].raiseError(new IllegalArgumentException(s"connectionTimeout cannot be less than 250ms, value: ${config.connectionTimeout}"))
  ...
  private[pool] def validateValidationTimeout[F[_]: Async](config: MySQLConfig): F[Unit] =
    if config.validationTimeout <= Duration.Zero then
      Async[F].raiseError(new IllegalArgumentException(s"validationTimeout cannot be less than 250ms, value: ${config.validationTimeout}"))
  ...
  private[pool] def validateIdleTimeout[F[_]: Async](config: MySQLConfig): F[Unit] =
    if config.idleTimeout < Duration.Zero then
      Async[F].raiseError(new IllegalArgumentException(s"idleTimeout cannot be negative, value: ${config.idleTimeout}"))
    else
      Async[F].unit
      
  private[pool] def validateMaxLifetime[F[_]: Async](config: MySQLConfig): F[Unit] =
    if config.maxLifetime <= Duration.Zero then
      Async[F].raiseError(new IllegalArgumentException(s"maxLifetime cannot be less than 30 seconds, value: ${config.maxLifetime}"))
  ...
  private[pool] def validateMaintenanceInterval[F[_]: Async](config: MySQLConfig): F[Unit] =
    if config.maintenanceInterval <= Duration.Zero then
      Async[F].raiseError(new IllegalArgumentException(s"maintenanceInterval cannot be less than 1 second, value: ${config.maintenanceInterval}"))
    else
      Async[F].unit
    ...
    config.leakDetectionThreshold match
      case Some(threshold) =>
        if threshold < 2.seconds then
          Async[F].raiseError(new IllegalArgumentException(s"leakDetectionThreshold cannot be less than 2 seconds, value: $threshold"))
  ...
  private[pool] def validateLogicalRelationship[F[_]: Async](config: MySQLConfig): F[Unit] =
    if config.idleTimeout > Duration.Zero && config.idleTimeout > config.maxLifetime then
      Async[F].raiseError(new IllegalArgumentException(s"idleTimeout (${config.idleTimeout}) cannot be greater than maxLifetime (${config.maxLifetime})"))
    else
      Async[F].unit
