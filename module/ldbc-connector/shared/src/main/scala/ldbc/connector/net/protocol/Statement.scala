...
object Statement:
  ...
  private[ldbc] trait ShareStatement[F[_]: Temporal](using ev: MonadError[F, Throwable]) extends Statement[F]:
    ...
    override def getGeneratedKeys(): F[Int] =
      autoGeneratedKeys.get.flatMap {
        case Statement.RETURN_GENERATED_KEYS => lastInsertId.get
        case Statement.NO_GENERATED_KEYS => ev.raiseError(new SQLException("Generated keys not requested. You need to specify Statement.RETURN_GENERATED_KEYS to Statement.executeUpdate(), Statement.executeLargeUpdate() or Connection.prepareStatement()."))
      }
        ...
        span.addAttributes(
          ...
          protocol.send(ComQueryPacket(sql, protocol.initialPacket.capabilityFlags, ListMap.empty)) *>
            protocol.receive(GenericResponsePackets.decoder(protocol.initialPacket.capabilityFlags)).flatMap {
              case result: OKPacket => lastInsertId.set(result.lastInsertId) *> updateCount.updateAndGet(_ => result.affectedRows)
              case error: ERRPacket => ev.raiseError(error.toException("Failed to execute query", sql))
