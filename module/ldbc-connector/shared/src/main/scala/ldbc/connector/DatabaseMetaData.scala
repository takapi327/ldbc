  ...
  private[ldbc] case class Impl[F[_]: Temporal: Exchange: Tracer](
    ...
    override def getProcedureColumns(
      ...
      if db.nonEmpty then
        ...
        conditionBuf.append(
          if databaseTerm.contains(DatabaseTerm.SCHEMA) then " SPECIFIC_SCHEMA LIKE ?"
    ...
    protected def prepareMetaDataSafeStatement(sql: String): F[PreparedStatement[F]] =
      for
        ...
        autoGeneratedKeys <- Ref[F].of[Statement.NO_GENERATED_KEYS | Statement.RETURN_GENERATED_KEYS](Statement.NO_GENERATED_KEYS)
