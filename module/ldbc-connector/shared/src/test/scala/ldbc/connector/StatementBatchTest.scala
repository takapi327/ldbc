...
class StatementBatchTest extends CatsEffectSuite:
  ...
  test("If the batch execution of Client PreparedStatement is successful, the result will be a -2 array of the number of queries that were batch executed.") {
    assertIO(
      connection.use { conn =>
        for
          statement <- conn.createStatement()
  ...
  test("If the batch execution of Server PreparedStatement is successful, the result will be a -2 array of the number of queries that were batch executed.") {
    assertIO(
      connection.use { conn =>
        for
          statement <- conn.createStatement()
