...
class StreamingResultSetTest extends FTestPlatform:
  ...
  class MockStreamingResultSet[F[_]: Async](
    ...
    val _ = (testColumns, testRecords, testServerVariables, testVersion, testIsClosed, testUseCursorFetch, testUseServerPrepStmts)
    ...
    def next(): F[Boolean] = testFetchSize.get.flatMap { size =>
      if (isCompleteAllFetch && currentCursor >= rows.length) {
        resetSequenceIdCalled += 1
        closeStmt()
      } else if (rows.isEmpty) {
        fetchRow(size) *> next()
      } else if (currentCursor >= rows.length) {
        fetchRow(size) *> next()
      } else {
        Async[F].delay {
    ...
    for
      ...
      results <- (1 to 6).toList.traverse { _ =>
        mockResultSet.next().flatMap { hasNext =>
          if (hasNext) mockResultSet.getString(2).map(Some(_))
          else IO.pure(None)
        }
      }
    yield {
