...
class SharedPreparedStatementTest extends SharedPreparedStatement[IO], FTestPlatform:
  ...
  override val currentResultSet: Ref[IO, Option[ResultSet]] = Ref.unsafe[IO, Option[ResultSet]](None)
  ...
  override def getGeneratedKeys(): IO[ResultSet] = IO.raiseError(new UnsupportedOperationException("Not implemented for test"))
  ...
  private def assertParameters(actual: SortedMap[Int, Parameter], expected: SortedMap[Int, Parameter]): IO[Unit] = {
    IO {
      ...
      actual.keys.foreach { key =>
        ...
        assert(assertParameter(actualParam, expectedParam),
  ...
  test("buildBatchQuery should throw exception for non-DML statements") {
    for {
      ...
      _ <- IO(assert(result.isLeft && result.left.exists(_.getMessage == "The batch query must be an INSERT, UPDATE, or DELETE statement.")))
    } yield ()
  }

  override def executeQuery(): IO[ResultSet] = 
