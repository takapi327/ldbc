...
class SQLStringContextUpdateTest extends CatsEffectSuite:
  ...
  test("The value generated when adding a record of AUTO_INCREMENT is returned.") {
    assertIO(
      connection.use { conn =>
        (for
          _ <- sql"CREATE TABLE `auto_inc_table`(`id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY, `c1` VARCHAR(255) NOT NULL)".update
          _ <- sql"INSERT INTO `auto_inc_table`(`id`, `c1`) VALUES (null, 'column 1')".update
          generated <- sql"INSERT INTO `auto_inc_table`(`id`, `c1`) VALUES (null, 'column 2')".updateReturningAutoGeneratedKey[Long]
          _         <- sql"DROP TABLE `auto_inc_table`".update
