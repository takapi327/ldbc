...
trait SystemSettingsCredentialsProvider[F[_]](using ev: MonadThrow[F]) extends AwsCredentialsProvider[F]:
  ...
  override def resolveCredentials(): F[AwsCredentials] =
    for
      ...
      accessKey <- ev.fromOption(accessKeyOpt, new SdkClientException(s"Unable to load credentials from system settings. Access key must be specified either via environment variable (${ SdkSystemSetting.AWS_ACCESS_KEY_ID }) or system property (${ SdkSystemSetting.AWS_ACCESS_KEY_ID.systemProperty })."))
      secretKey <- ev.fromOption(secretKeyOpt, new SdkClientException(s"Unable to load credentials from system settings. Secret key must be specified either via environment variable (${ SdkSystemSetting.AWS_SECRET_ACCESS_KEY }) or system property (${ SdkSystemSetting.AWS_SECRET_ACCESS_KEY.systemProperty })."))
