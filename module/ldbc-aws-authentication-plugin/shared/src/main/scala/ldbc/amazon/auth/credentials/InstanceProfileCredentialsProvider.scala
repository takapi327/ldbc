  ...
  private def parseCredentialsResponse(jsonBody: String, roleName: String): F[AwsCredentials] =
    Concurrent[F].fromEither(
      ...
      if (response.Code == "Success") {
        Concurrent[F].pure(AwsSessionCredentials(
