...
object StsClient:
  ...
  private case class Impl[F[_]: Concurrent]() extends StsClient[F]:
    ...
    def assumeRoleWithWebIdentity(
      ...
      for
        ...
        requestBody = buildRequestBody(request.copy(
  ...
  private def validateHttpResponse[F[_]: MonadThrow](response: HttpResponse): F[Unit] =
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      MonadThrow[F].raiseError(new StsException(
  ...
  private def parseAssumeRoleResponse[F[_]: MonadThrow](xmlBody: String): F[AssumeRoleWithWebIdentityResponse] =
    MonadThrow[F].catchNonFatal {
      ...
      if (accessKeyId.isEmpty) throw new StsException("AccessKeyId not found in STS response")
      if (secretAccessKey.isEmpty) throw new StsException("SecretAccessKey not found in STS response")
      if (sessionToken.isEmpty) throw new StsException("SessionToken not found in STS response")
      if (assumedRoleArn.isEmpty) throw new StsException("AssumedRoleArn not found in STS response")
