    ...
    "The query statement generated from Table is equal to the specified query statement." in {
      table.select[Column[Long]](_.p1).statement === "SELECT `p1` FROM test" and
        ...
        table
          ...
          .where(_.p1 >= 1)
          .and(_.p2 === "test")
          .statement === "SELECT `p1` FROM test WHERE p1 >= ? AND p2 = ?" and
        table
          .select[Column[Long]](_.p1)
          .where(v => v.p1 > 1 || v.p2 === "test")
          .statement === "SELECT `p1` FROM test WHERE (p1 > ? OR p2 = ?)" and
        table
          .select[Column[Long]](_.p1)
          .where(v => v.p1 > 1 && v.p2 === "test")
          .or(_.p3 === "test")
          .statement === "SELECT `p1` FROM test WHERE (p1 > ? AND p2 = ?) OR p3 = ?" and
        table.select[Column[Long]](_.p1).groupBy(p1 => p1).statement === "SELECT `p1` FROM test GROUP BY p1" and
        table
          .select[Column[Long]](_.p1)
          .groupBy(p1 => p1)
          .having(_ < 1)
          .statement === "SELECT `p1` FROM test GROUP BY p1 HAVING p1 < ?" and
