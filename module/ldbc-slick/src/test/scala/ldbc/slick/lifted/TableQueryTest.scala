...
class TableQueryTest extends AnyFlatSpec:
  ...
  it should "The select query statement generated from Table is equal to the specified query statement." in {
    ...
    assert(TableQueryTest.cityQuery6 === "select exists(select `District`, `CountryCode`, `Name`, `ID`, `Population` from `city`)")
    assert(
  ...
  it should "The insert query statement generated from Table is equal to the specified query statement." in {
    assert(TableQueryTest.countryLanguageQuery1 === "insert into `countrylanguage` (`CountryCode`,`Language`,`IsOfficial`,`Percentage`)  values (?,?,?,?)")
    assert(TableQueryTest.countryLanguageQuery2 === "insert into `countrylanguage` (`CountryCode`,`Language`,`IsOfficial`,`Percentage`)  values (?,?,?,?)")
    assert(TableQueryTest.countryLanguageQuery3 === "insert into `countrylanguage` (`CountryCode`,`Language`)  values (?,?)")
    assert(TableQueryTest.cityQuery8 === "insert into `city` (`ID`,`Name`,`CountryCode`,`District`,`Population`)  values (?,?,?,?,?)")
  }
  ...
  it should "The update query statement generated from Table is equal to the specified query statement." in {
    ...
    assert(TableQueryTest.countryLanguageQuery5 === "update `countrylanguage` set `CountryCode` = ?, `Language` = ? where `countrylanguage`.`CountryCode` = 'JPN'")
...
object TableQueryTest:
  ...
  val countryQuery4 = countries.filter(_.population > 126713999).filter(_.population < 126714001).result.headOption.statements.head
  ...
  val countryQuery8 = countries.filter { country =>
    List(
  ...
  val countryQuery9 = countries.filterOpt(Some("JPN"))(_.code === _).filterOpt(none)(_.code === _).result.statements.head
  val countryQuery10 = countries.filterIf(true)(_.code === "JPN").filterIf(false)(_.code === "JOR").result.statements.head
  ...
  val cityQuery8 = ((cities returning cities.map(_.id) into ((ciry, id) => ciry.copy(id = id))) += City(0, "", "", "", 0)).statements.head
  ...
  val countryLanguageQuery1 = (countryLanguages += CountryLanguage("JPN", "", CountryLanguage.IsOfficial.T, BigDecimal.decimal(2.2))).statements.head
  ...
  val countryLanguageQuery4 = countryLanguages.map(c => (c.countryCode, c.language)).update(("JPN", "Update")).statements.head
  val countryLanguageQuery5 = countryLanguages.filter(_.countryCode === "JPN").map(c => (c.countryCode, c.language)).update(("JPN", "Update")).statements.head
