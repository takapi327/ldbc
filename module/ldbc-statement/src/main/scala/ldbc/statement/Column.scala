    ...
    new Column[(A, B)]:
      ...
      override def opt:    Column[Option[(A, B)]] =
        val decoder = new Decoder[Option[(A, B)]]:
          ...
          override def decode(index: Int): Either[Decoder.Error, ResultSetIO[Option[(A, B)]]] =
            for
              v1 <- self.opt.decoder.decode(index)
              v2 <- fb.opt.decoder.decode(index + self.decoder.offset)
            yield v1.flatMap(opt1 => v2.map(opt2 => opt1.flatMap(a => opt2.map(b => (a, b)))))
...
object Column extends TwiddleSyntax[Column]:
  ...
  case class Pure[A](value: A) extends Column[A]:
    ...
    override def decoder:          Decoder[A]     = new Decoder[A]:
      ...
      override def decode(index: Int): Either[Decoder.Error, ResultSetIO[A]] = Right(ResultSetIO.pure(value))
