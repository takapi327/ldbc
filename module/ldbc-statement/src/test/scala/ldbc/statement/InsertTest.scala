...
class InsertTest extends AnyFlatSpec:
  ...
  it should "construct onDuplicateKeyUpdate with column reference" in {
    ...
    assert(duplicateKeyUpdate.statement === "INSERT INTO table_name (column1, column2) VALUES (?, ?) ON DUPLICATE KEY UPDATE `column1` = VALUES(`column1`)")
    assert(duplicateKeyUpdate.params.length === 2)
    assert(combined.statement === "INSERT INTO table_name (column1, column2) VALUES (?, ?) ON DUPLICATE KEY UPDATE `column1` = VALUES(`column1`) AND WHERE is_active = ?")
    assert(combined.params.length === 3)
  }
  ...
  it should "construct onDuplicateKeyUpdate with explicit value" in {
    ...
    assert(duplicateKeyUpdate.statement === "INSERT INTO table_name (column1, column2) VALUES (?, ?) ON DUPLICATE KEY UPDATE `column1` = ?")
  ...
  it should "support onDuplicateKeyUpdate on Values" in {
    ...
    assert(duplicateKeyUpdate.statement === "INSERT INTO test_table (`id`, `name`) VALUES (?,?) ON DUPLICATE KEY UPDATE `name` = VALUES(`name`)")
    assert(duplicateKeyUpdate.params.length === 2)
  }
  ...
  it should "support onDuplicateKeyUpdate with explicit value on Values" in {
    ...
    assert(duplicateKeyUpdate.statement === "INSERT INTO test_table (`id`, `name`) VALUES (?,?) ON DUPLICATE KEY UPDATE `name` = ?")
    assert(duplicateKeyUpdate.params.length === 3)
