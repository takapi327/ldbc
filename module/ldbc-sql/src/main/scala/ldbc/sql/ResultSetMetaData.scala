 ...
 * The following code fragment creates the ResultSet object rs, creates the ResultSetMetaData object rsmd, and uses rsmd
 * to find out how many columns rs has and whether the first column in rs can be used in a WHERE clause.
 *
 * @tparam F
 *   The effect type
 */
trait ResultSetMetaData[F[_]]:
   ...
   * is the length in bytes. For the ROWID datatype, this is the length in bytes. 0 is returned for data types where the
  ...
  /**
   * Gets the designated column's number of digits to right of the decimal point. 0 is returned for data types where the
   ...
   * ResultSet.getObject is called to retrieve a value from the column. ResultSet.getObject may return a subclass of the
