...
trait Statement[F[_]]:
  ...
  /** Executes the given SQL statement and signals the driver with the given flag about whether the auto-generated keys
    ...
    *   constants: Statement.RETURN_GENERATED_KEYS Statement.NO_GENERATED_KEYS
    * @return
    *   either (1) the row count for SQL Data Manipulation Language (DML) statements or (2) 0 for SQL statements that
    *   return nothing
    */
  ...
  /** Executes the given SQL statement, which may return multiple results, and signals the driver that any
    ...
    *   getGeneratedKeys; one of the following constants: Statement.RETURN_GENERATED_KEYS or Statement.NO_GENERATED_KEYS
    * @return
    *   true if the first result is a ResultSet object; false if it is an update count or there are no results
    */
...
object Statement:
  ...
  def apply[F[_]: Sync](statement: java.sql.Statement): Statement[F] = new Statement[F]:
    override def executeQuery(sql: String): F[ResultSet[F]] =
      Sync[F].blocking(statement.executeQuery(sql)).map(ResultSet[F](_))
