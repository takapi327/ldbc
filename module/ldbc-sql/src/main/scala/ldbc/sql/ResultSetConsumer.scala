...
object ResultSetConsumer:
  ...
  given [F[_], T](using consumer: ResultSetConsumer[F, Option[T]], error: MonadError[F, Throwable]): ResultSetConsumer[F, T] with
  ...
  given [F[_]: Monad, T, S[_]: Traverse: Alternative](using resultSetKleisli: Kleisli[F, ResultSet[F], T]): ResultSetConsumer[F, S[T]] with
    override def consume(resultSet: ResultSet[F]): F[S[T]] = Monad[F].whileM[S, T](resultSet.next())(resultSetKleisli.run(resultSet))
