...
trait Connection[F[_]]:
  ...
  /** Creates a PreparedStatement object that will generate ResultSet objects with the given type and concurrency. This
    ...
    */
  def prepareStatement(sql: String, resultSetType: ResultSet.Type, resultSetConcurrency: ResultSet.Concur): F[PreparedStatement[F]]
  ...
  /** Creates a Statement object that will generate ResultSet objects with the given type, concurrency, and holdability.
    ...
    */
  def createStatement(resultSetType: ResultSet.Type, resultSetConcurrency: ResultSet.Concur, resultSetHoldability: ResultSet.Holdability): F[Statement[F]]
...
object Connection:
  ...
  def apply[F[_]: Sync](connection: java.sql.Connection): Connection[F] = new Connection[F]:
    ...
    override def createStatement(): F[Statement[F]] =
      Sync[F].blocking(connection.createStatement()).map(Statement[F](_))

    override def prepareStatement(sql: String): F[PreparedStatement[F]] =
      Sync[F].blocking(connection.prepareStatement(sql)).map(PreparedStatement[F](_))
    ...
    override def createStatement(resultSetType: ResultSet.Type, resultSetConcurrency: ResultSet.Concur): F[Statement[F]] =
      Sync[F].blocking(connection.createStatement(resultSetType.code, resultSetConcurrency.code)).map(Statement[F](_))

    override def prepareStatement(sql: String, resultSetType: ResultSet.Type, resultSetConcurrency: ResultSet.Concur): F[PreparedStatement[F]] =
      Sync[F]
        .blocking(connection.prepareStatement(sql, resultSetType.code, resultSetConcurrency.code))
        .map(PreparedStatement[F](_))
    ...
    override def prepareStatement(
      ...
      .blocking(connection.prepareStatement(sql, resultSetType.code, resultSetConcurrency.code, resultSetHoldability.code))
      .map(PreparedStatement[F](_))
    override def prepareStatement(sql: String, autoGeneratedKeys: Int): F[PreparedStatement[F]] =
      Sync[F].blocking(connection.prepareStatement(sql, autoGeneratedKeys)).map(PreparedStatement[F](_))
    override def prepareStatement(sql: String, columnIndexes: Array[Int]): F[PreparedStatement[F]] =
      Sync[F].blocking(connection.prepareStatement(sql, columnIndexes)).map(PreparedStatement[F](_))
    override def prepareStatement(sql: String, columnNames: Array[String]): F[PreparedStatement[F]] =
      Sync[F].blocking(connection.prepareStatement(sql, columnNames)).map(PreparedStatement[F](_))
