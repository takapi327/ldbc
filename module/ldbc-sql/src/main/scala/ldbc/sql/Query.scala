...
trait Query[F[_], T]:
  ...
  def flatMap[B](f: T => Query[F, B])(using Monad[F]): Query[F, B] =
    new Query[F, B]:
      ...
      override def readOnly(connection: Connection[F]): F[B] = Monad[F].flatMap(Query.this.readOnly(connection))(a => f(a).readOnly(connection))
      override def autoCommit(connection: Connection[F]): F[B] = Monad[F].flatMap(Query.this.autoCommit(connection))(a => f(a).autoCommit(connection))
      override def transaction(connection: Connection[F]): F[B] = Monad[F].flatMap(Query.this.transaction(connection))(a => f(a).transaction(connection))
      override def rollback(connection: Connection[F]): F[B] = Monad[F].flatMap(Query.this.rollback(connection))(a => f(a).rollback(connection))
  ...
  given [F[_]: Monad]: Monad[[T] =>> Query[F, T]] with
    ...
    override def flatMap[A, B](fa: Query[F, A])(f: A => Query[F, B]): Query[F, B] =
      new Query[F, B]:
        ...
        override def readOnly(connection: Connection[F]): F[B] = Monad[F].flatMap(fa.readOnly(connection))(a => f(a).readOnly(connection))
        override def autoCommit(connection: Connection[F]): F[B] = Monad[F].flatMap(fa.autoCommit(connection))(a => f(a).autoCommit(connection))
        override def transaction(connection: Connection[F]): F[B] = Monad[F].flatMap(fa.transaction(connection))(a => f(a).transaction(connection))
        override def rollback(connection: Connection[F]): F[B] = Monad[F].flatMap(fa.rollback(connection))(a => f(a).rollback(connection))
    ...
    override def tailRecM[A, B](a: A)(f: A => Query[F, Either[A, B]]): Query[F, B] =
      new Query[F, B]:
        ...
        override def autoCommit(connection: Connection[F]): F[B] = Monad[F].tailRecM(a)(a => f(a).autoCommit(connection))
        override def transaction(connection: Connection[F]): F[B] = Monad[F].tailRecM(a)(a => f(a).transaction(connection))
