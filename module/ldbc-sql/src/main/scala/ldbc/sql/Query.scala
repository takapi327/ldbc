...
trait Query[F[_], T]:
   ...
   */
  def autoCommit(connection: Connection[F])(using LogHandler[F]): F[T]
  ...
  given [F[_]: Monad]: Monad[[T] =>> Query[F, T]] with
    override def pure[A](x: A): Query[F, A] =
      new Query[F, A]:
        ...
        override def readOnly(connection:    Connection[F])(using LogHandler[F]): F[A] = Monad[F].pure(x)
        override def autoCommit(connection:  Connection[F])(using LogHandler[F]): F[A] = Monad[F].pure(x)
        override def transaction(connection: Connection[F])(using LogHandler[F]): F[A] = Monad[F].pure(x)
        override def rollback(connection:    Connection[F])(using LogHandler[F]): F[A] = Monad[F].pure(x)
