...
class KleisliInterpreter[F[_]: Sync]:
  ...
  def handleErrorWith[G[_], J, A](interpreter: G ~> ([T] =>> Kleisli[F, J, T]))(fa: Free[G, A])(f: Throwable => Free[G, A]): Kleisli[F, J, A] = Kleisli(j =>
    Sync[F].handleErrorWith(fa.foldMap(interpreter).run(j))(f.andThen(_.foldMap(interpreter).run(j)))
  )
  def forceR[G[_], J, A, B](interpreter: G ~> ([T] =>> Kleisli[F, J, T]))(fa: Free[G, A])(fb: Free[G, B]): Kleisli[F, J, B] = Kleisli(j =>
    Sync[F].forceR(fa.foldMap(interpreter).run(j))(fb.foldMap(interpreter).run(j))
  )
  def uncancelable[G[_], J, A](interpreter: G ~> ([T] =>> Kleisli[F, J, T]), capture: Poll[F] => Poll[[T] =>> Free[G, T]])(body: Poll[[T] =>> Free[G, T]] => Free[G, A]): Kleisli[F, J, A] = Kleisli(j =>
    Sync[F].uncancelable(body.compose(capture).andThen(_.foldMap(interpreter).run(j)))
  )
  def poll[G[_], J, A](interpreter: G ~> ([T] =>> Kleisli[F, J, T]))(mpoll: Any, fa: Free[G, A]): Kleisli[F, J, A] = Kleisli(j =>
    mpoll.asInstanceOf[Poll[F]].apply(fa.foldMap(interpreter).run(j))
  )
  def onCancel[G[_], J, A](interpreter: G ~> ([T] =>> Kleisli[F, J, T]))(fa: Free[G, A], fin: Free[G, Unit]): Kleisli[F, J, A] = Kleisli(j =>
    Sync[F].onCancel(fa.foldMap(interpreter).run(j), fin.foldMap(interpreter).run(j))
  )
  ...
  trait ConnectionInterpreter extends ConnectionOp.Visitor[[A] =>> Kleisli[F, Connection[F], A]]:
    ...
    override def handleErrorWith[A](fa: ConnectionIO[A])(f: Throwable => ConnectionIO[A]): Kleisli[F, Connection[F], A] = outer.handleErrorWith(this)(fa)(f)
    override def forceR[A, B](fa: ConnectionIO[A])(fb: ConnectionIO[B]): Kleisli[F, Connection[F], B] = outer.forceR(this)(fa)(fb)
    ...
    override def setAutoCommit(autoCommit: Boolean): Kleisli[F, Connection[F], Unit] = primitive(_.setAutoCommit(autoCommit))
    ...
    override def setReadOnly(isReadOnly: Boolean): Kleisli[F, Connection[F], Unit] = primitive(_.setReadOnly(isReadOnly))
    override def prepareStatement(sql: String, resultSetType: Int, resultSetConcurrency: Int): Kleisli[F, Connection[F], PreparedStatement[?]] =
      primitive[Connection[F], PreparedStatement[F]](_.prepareStatement(sql, resultSetType, resultSetConcurrency))
        .asInstanceOf[Kleisli[F, Connection[F], PreparedStatement[?]]]
    override def prepareStatement(sql: String, autoGeneratedKeys: Int): Kleisli[F, Connection[F], PreparedStatement[?]] =
  ...
  trait PreparedStatementInterpreter extends PreparedStatementOp.Visitor[[A] =>> Kleisli[F, PreparedStatement[F], A]]:
    ...
    override def suspend[A](hint: Sync.Type)(thunk: => A): Kleisli[F, PreparedStatement[F], A] = outer.suspend(hint)(thunk)
    override def canceled: Kleisli[F, PreparedStatement[F], Unit] = outer.canceled[PreparedStatement[F]]
    override def handleErrorWith[A](fa: PreparedStatementIO[A])(f: Throwable => PreparedStatementIO[A]): Kleisli[F, PreparedStatement[F], A] = outer.handleErrorWith(this)(fa)(f)
    override def forceR[A, B](fa: PreparedStatementIO[A])(fb: PreparedStatementIO[B]): Kleisli[F, PreparedStatement[F], B] = outer.forceR(this)(fa)(fb)
    override def uncancelable[A](body: Poll[PreparedStatementIO] => PreparedStatementIO[A]): Kleisli[F, PreparedStatement[F], A] = outer.uncancelable(this, PreparedStatementIO.capturePoll)(body)
    override def poll[A](poll: Any, fa: PreparedStatementIO[A]): Kleisli[F, PreparedStatement[F], A] = outer.poll(this)(poll, fa)
    override def onCancel[A](fa: PreparedStatementIO[A], fin: PreparedStatementIO[Unit]): Kleisli[F, PreparedStatement[F], A] = outer.onCancel(this)(fa, fin)

    override def setNull(index: Int, sqlType: Int): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setNull(index, sqlType))
    override def setBoolean(index: Int, value: Boolean): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setBoolean(index, value))
    override def setByte(index: Int, value: Byte): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setByte(index, value))
    override def setShort(index: Int, value: Short): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setShort(index, value))
    override def setInt(index: Int, value: Int): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setInt(index, value))
    override def setLong(index: Int, value: Long): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setLong(index, value))
    override def setFloat(index: Int, value: Float): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setFloat(index, value))
    override def setDouble(index: Int, value: Double): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setDouble(index, value))
    override def setBigDecimal(index: Int, value: BigDecimal): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setBigDecimal(index, value))
    override def setString(index: Int, value: String): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setString(index, value))
    override def setBytes(index: Int, value: Array[Byte]): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setBytes(index, value))
    override def setDate(index: Int, value: LocalDate): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setDate(index, value))
    ...
    override def setObject(index: Int, value: Object): Kleisli[F, PreparedStatement[F], Unit] = primitive(_.setObject(index, value))
    ...
    override def executeLargeUpdate(): Kleisli[F, PreparedStatement[F], Long] = primitive[PreparedStatement[F], Long](_.executeLargeUpdate())
  ...
  trait ResultSetInterpreter extends ResultSetOp.Visitor[[A] =>> Kleisli[F, ResultSet[F], A]]:
    ...
    override def getTimestamp(columnIndex: Int): Kleisli[F, ResultSet[F], LocalDateTime] = primitive(_.getTimestamp(columnIndex))
    override def getString(columnLabel: String): Kleisli[F, ResultSet[F], String] = primitive(_.getString(columnLabel))
    override def getBoolean(columnLabel: String): Kleisli[F, ResultSet[F], Boolean] = primitive(_.getBoolean(columnLabel))
    ...
    override def getBytes(columnLabel: String): Kleisli[F, ResultSet[F], Array[Byte]] = primitive(_.getBytes(columnLabel))
    ...
    override def getTimestamp(columnLabel: String): Kleisli[F, ResultSet[F], LocalDateTime] = primitive(_.getTimestamp(columnLabel))
    override def getMetaData(): Kleisli[F, ResultSet[F], ResultSetMetaData] = primitive(_.getMetaData())
    override def getBigDecimal(columnIndex: Int): Kleisli[F, ResultSet[F], BigDecimal] = primitive(_.getBigDecimal(columnIndex))
    override def getBigDecimal(columnLabel: String): Kleisli[F, ResultSet[F], BigDecimal] = primitive(_.getBigDecimal(columnLabel))
