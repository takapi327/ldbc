...
case class Database[F[_]: Sync](
  ...
  def autoCommit[T](connectionKleisli: Kleisli[F, Connection[F], T]): F[T] =
    connection(connection => connection.setReadOnly(false) >> connection.setAutoCommit(true) >> connectionKleisli.run(connection))
  ...
  def transaction[T](connectionKleisli: Kleisli[F, Connection[F], T]): F[T] =
    connection(connection => connection.setReadOnly(false) >> connection.setAutoCommit(false) >> Resource.makeCase(Sync[F].pure(connection)) {
  ...
  def rollback[T](connectionKleisli: Kleisli[F, Connection[F], T]): F[T] =
    connection(connection => connection.setAutoCommit(false) >> connectionKleisli.run(connection) <* connection.rollback())
