...
object DBIO extends ParamBinder:
  ...
  final case class Returning[A](statement: String, params: List[Parameter.Dynamic], decoder: Decoder[A]) extends DBIOA[A]
        ...
        override def apply[A](fa: DBIOA[A]): F[A] =
          fa match
                ...
                result <- {
                  ...
                  while resultSet.next() do
                    decoder.decode(resultSet, 1) match
                      ...
                      case Left(error) => throw new ldbc.dsl.exception.DecodeFailureException(error.message, decoder.offset, statement, error.cause)
            ...
            case Sequence(statements) =>
              ...
              yield result)
                .onError(ex => connection.logHandler.run(LogEvent.ProcessingFailure(statements.mkString("\n"), List.empty, ex))) <*
