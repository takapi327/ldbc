...
object DBIO:
  ...
  def queryA[A](
    ...
    (for
      ...
      resultSet        <- ConnectionIO.embed(prepareStatement, PreparedStatementIO.paramBind(params) *> PreparedStatementIO.executeQuery())
  ...
  def update(
    ...
    (for
      prepareStatement <- ConnectionIO.prepareStatement(statement)
      result           <- ConnectionIO.embed(prepareStatement, PreparedStatementIO.paramBind(params) *> PreparedStatementIO.executeUpdate())
      _                <- ConnectionIO.embed(prepareStatement, PreparedStatementIO.close())
    yield result).onError { ex =>
      ConnectionIO.performLogging(LogEvent.ProcessingFailure(statement, params.map(_.value), ex))
    } <*
  ...
  def returning[A](
    ...
    (for
      ...
      resultSet <- ConnectionIO.embed(prepareStatement, PreparedStatementIO.paramBind(params) *> PreparedStatementIO.executeUpdate() *> PreparedStatementIO.getGeneratedKeys())
      result           <- ConnectionIO.embed(resultSet, ResultSetIO.unique(statement, decoder))
      _                <- ConnectionIO.embed(prepareStatement, PreparedStatementIO.close())
    yield result).onError { ex =>
      ConnectionIO.performLogging(LogEvent.ProcessingFailure(statement, params.map(_.value), ex))
  ...
  implicit val syncDBIO: Sync[DBIO] =
    new Sync[DBIO]:
      ...
      override def handleErrorWith[A](fa: DBIO[A])(f: Throwable => DBIO[A]): DBIO[A] = ConnectionIO.handleErrorWith(fa)(f)
  ...
  private[ldbc] class Ops[A](dbio: DBIO[A]):
    ...
    def run[F[_]: Sync](connection: Connection[F]): F[A] =
      dbio.foldMap(new KleisliInterpreter[F](connection.logHandler).ConnectionInterpreter)
        .run(connection)
    ...
    def rollback[F[_] : Sync](connection: Connection[F]): F[A] =
      (ConnectionIO.setReadOnly(false) *> ConnectionIO.setAutoCommit(false) *> dbio <* ConnectionIO.rollback() <* ConnectionIO.setAutoCommit(true))
        .foldMap(new KleisliInterpreter[F](connection.logHandler).ConnectionInterpreter)
        .run(connection)
