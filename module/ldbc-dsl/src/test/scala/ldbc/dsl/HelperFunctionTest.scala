...
class HelperFunctionTest extends munit.CatsEffectSuite:
  ...
  test("The statement that constructs VALUES with multiple values of the same type will be the same as the string specified.") {
  ...
  test("A statement that constructs VALUES in multiple sql is the same as the specified string.") {
    ...
    val sql = q"INSERT INTO `table` (`column1`, `column2`) VALUES" ++ comma(vs.map(v => parentheses(p"${v.c1},${v.c2}")))
    assertEquals(sql.statement, "INSERT INTO `table` (`column1`, `column2`) VALUES(?,?),(?,?)")
  }
  ...
  test("A statement that is constructed by ANDing Where clauses with multiple values is the same as the string specified.") {
    val sql = q"SELECT * FROM `table` " ++ where("AND", q"`column1` = 1", q"`column2` = 2")
    assertEquals(sql.statement, "SELECT * FROM `table` WHERE `column1` = 1 AND `column2` = 2")
  }
  ...
  test("A statement that is constructed by ORing Where clauses with multiple values is the same as the string specified.") {
