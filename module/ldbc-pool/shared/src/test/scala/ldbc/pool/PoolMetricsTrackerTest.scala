...
class PoolMetricsTrackerTest extends FxSuite:
  ...
  test("noop tracker should always return empty metrics") {
    val tracker = PoolMetricsTracker.noop[Fx]
  ...
  test("in-memory tracker should handle gauge updates") {
    for
      ...
      _       <- tracker.updateGauge("active", 7)
      metrics <- tracker.getMetrics
  ...
  test("in-memory tracker should maintain sliding window of duration samples") {
    for
      tracker <- PoolMetricsTracker.inMemory[Fx]
      _       <- (1 to 110).toList.parTraverseN(10)(i => tracker.recordAcquisition(i.millis))
      metrics <- tracker.getMetrics
    yield
      assert(60400.micros < metrics.acquisitionTime || metrics.acquisitionTime < 60500.micros)
