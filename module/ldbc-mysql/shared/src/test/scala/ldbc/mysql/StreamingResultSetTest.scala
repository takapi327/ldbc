...
class StreamingResultSetTest extends FTestPlatform:
  ...
  class MockStreamingResultSet(
    ...
    // Simulate fetchRow
    def fetchRow(size: Int): Fx[Unit] = Fx.delay {
    ...
    // Simulate closeStmt
    def closeStmt(): Fx[Boolean] = Fx.delay {
    ...
    def next(): Fx[Boolean] = testFetchSize.get.flatMap { size =>
    ...
    // Simulate getString
    def getString(columnIndex: Int): Fx[String] = Fx.delay {
      currentRow match {
        case Some(row) =>
