    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Byte, Byte)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Short, Short)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Int, Int)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Long, Long)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded <- resultSet.whileM[List, (String, String)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Float, Float)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Double, Double)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded <- resultSet.whileM[List, (BigDecimal, BigDecimal)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (LocalDate, LocalDate)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (LocalTime, LocalTime)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded <- resultSet.whileM[List, (LocalDateTime, LocalDateTime)] {
                       for
        ...
        for
          ...
          resultSet <-
            statement.setBytes(1, Array[Byte](98, 105, 110, 97, 114, 121, 0, 0, 0, 0)) *> statement
              .executeQuery()
          decoded <- resultSet.whileM[List, (String, Array[Byte])] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Boolean, Boolean)] {
                       for
    ...
    assertFx(
      connection.use { conn =>
        for
          ...
          decoded   <- resultSet.whileM[List, (Array[Byte], String, Array[Byte])] {
                       for
                         v1 <- resultSet.getBytes(1)
                         v2 <- resultSet.getString(2)
