...
class ParameterTest extends FTestPlatform:
  ...
  test("String parameter special cases") {
    ...
    val specialChars = "\t\r\n\b"
    assertEquals(QueryRenderer.render(Parameter.string(specialChars), noBackslashEscapes = false), "'\t\\r\\n\\b'")

    // Unicode string
    val unicodeString = "Hello 世界 🌍"
    assertEquals(QueryRenderer.render(Parameter.string(unicodeString), noBackslashEscapes = false), s"'$unicodeString'")
  }
