...
class ResultSetTest extends FTestPlatform:
  ...
  test("SQLException occurs when accessing the ResultSet after closing it.") {
    for
      ...
      _         <- resultSet.close()
      _         <- interceptFx[SQLException](resultSet.next())
      _         <- resultSet.close()
      _         <- interceptFx[SQLException](resultSet.getLong(1))
    yield ()
  }
  ...
  test("ResultSet should return the correct value for getInt") {
    for
      resultSet <- buildResultSet(
                     ...
                     Vector(ResultSetRowPacket.fromStrings(Some("1"), Some("2"), None)),
                     Version(0, 0, 0)
                   )
      hasNext <- resultSet.next()
      c1      <- if hasNext then resultSet.getInt(1) else Fx.pure(0)
      c2      <- if hasNext then resultSet.getInt("c2") else Fx.pure(0)
      c3      <- if hasNext then resultSet.getInt(3) else Fx.pure(0)
  ...
  test("ResultSet should return the correct value for getDouble") {
    for
      resultSet <- buildResultSet(
                     ...
                     Vector(ResultSetRowPacket.fromStrings(Some("1.1"), Some("2.2"), None)),
                     Version(0, 0, 0)
                   )
      result <- collectRows(resultSet) { rs =>
                  for
  ...
  test("ResultSet should return the correct value for getBoolean") {
    for
      resultSet <- buildResultSet(
                     ...
                     Vector(ResultSetRowPacket.fromStrings(Some("1"), Some("0"), None)),
                     Version(0, 0, 0)
                   )
      result <- collectRows(resultSet) { rs =>
                  for
  ...
  test("ResultSet should return the correct value for getDate") {
    for
      resultSet <- buildResultSet(
                     ...
                     Vector(ResultSetRowPacket.fromStrings(Some("2023-01-01"), Some("2023-01-02"), None)),
                     Version(0, 0, 0)
                   )
      result <- collectRows(resultSet) { rs =>
                  for
  ...
  test("ResultSet should return the correct value for getTime") {
    for
      resultSet <- buildResultSet(
                     ...
                     Vector(ResultSetRowPacket.fromStrings(Some("12:34:56"), Some("12:34:57"), None)),
                     Version(0, 0, 0)
                   )
      result <- collectRows(resultSet) { rs =>
                  for
  ...
  test("Multiple records can be retrieved by looping through them with next.") {
    for
      resultSet <- buildResultSet(
                     ...
                     Vector(
                       ResultSetRowPacket.fromStrings(Some("1"), Some("2"), None),
                       ResultSetRowPacket.fromStrings(Some("3"), Some("4"), None),
                       ResultSetRowPacket.fromStrings(Some("5"), Some("6"), None),
                       ResultSetRowPacket.fromStrings(Some("7"), Some("8"), None)
                     ),
                     Version(0, 0, 0)
                   )
      result <- collectRows(resultSet) { rs =>
  ...
  test(
    ...
    for
      resultSet <- buildResultSet(
                     ...
                     Vector(ResultSetRowPacket.fromStrings(Some("2023-01-01 12:34:56"))),
                     Version(0, 0, 0)
                   )
  ...
  test(
    ...
    for
      ...
      _              <- collectRows(resultSet)(_ => Fx.unit)
  ...
  test(
    ...
    for
      ...
      _ <- interceptFx[SQLException](resultSet.beforeFirst())
    yield ()
  }
  ...
  test(
    ...
    for
      ...
      _ <- interceptFx[SQLException](resultSet.afterLast())
    yield ()
  }
  ...
  test("When the type of ResultSet is TYPE_FORWARD_ONLY, the cursor position operation by first throws SQLException.") {
    for
      ...
      _ <- interceptFx[SQLException](resultSet.first())
    yield ()
  }
  ...
  test(
    ...
    for
      resultSet <- buildResultSet(
                     Vector(column("c1", ColumnDataType.MYSQL_TYPE_TIMESTAMP)),
                     Vector(
                       ResultSetRowPacket.fromStrings(Some("2023-01-01 12:34:56")),
                       ResultSetRowPacket.fromStrings(Some("2023-01-01 12:34:56"))
                     ),
                     Version(0, 0, 0),
                     ResultSet.TYPE_SCROLL_INSENSITIVE,
  ...
  test("When the type of ResultSet is TYPE_FORWARD_ONLY, the cursor position operation by last throws SQLException.") {
    for
      ...
      _ <- interceptFx[SQLException](resultSet.last())
    yield ()
  }
  ...
  test(
    ...
    for
      resultSet <- buildResultSet(
                     Vector(column("c1", ColumnDataType.MYSQL_TYPE_TIMESTAMP)),
                     Vector(ResultSetRowPacket.fromStrings(Some("1")), ResultSetRowPacket.fromStrings(Some("2"))),
                     Version(0, 0, 0),
                     ResultSet.TYPE_SCROLL_INSENSITIVE,
                     ResultSet.CONCUR_READ_ONLY
                   )
  ...
  test(
    ...
    for
      ...
      _ <- interceptFx[SQLException](resultSet.absolute(0))
    yield ()
  }
  ...
  test(
    ...
    for
      ...
      _ <- interceptFx[SQLException](resultSet.relative(0))
    yield ()
  }
  ...
  test(
    ...
    for
      ...
      _ <- interceptFx[SQLException](resultSet.previous())
    yield ()
  }
