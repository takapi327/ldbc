...
case class CallableStatementImpl[F[_]](
    ...
    checkClosed() *>
      ...
      exchange[F, ResultSet[F]](
        ...
        for
                           ...
                           executeCallStatement(span).flatMap { resultSets =>
                             resultSets.headOption match
                               case None =>
                                 ...
                                 yield resultSet
                               case Some(resultSet) =>
                                 currentResultSet.update(_ => Some(resultSet)) *> F.pure(resultSet)
                           } <* retrieveOutParams()
                         else
          ...
          _ <- resultSet match
                 case rs: (ResultSetImpl[F] @unchecked) =>
  ...
  private def receiveUntilOkPacket(resultSets: Vector[ResultSetImpl[F]]): F[Vector[ResultSetImpl[F]]] =
    protocol.receive(ColumnsNumberPacket.decoder(protocol.initialPacket.capabilityFlags)).flatMap {
      case _: OKPacket                 => F.pure(resultSets)
      case error: ERRPacket            => F.raiseError(error.toException(Some(sql), None))
    ...
    if paramInfo.numParameters > 0 && parameters.nonEmpty then
      ...
      checkClosed() *>
        ...
        receiveQueryResult().flatMap {
          case resultSet: (ResultSetImpl[F] @unchecked) => outputParameterResult.update(_ => Some(resultSet))
        } *>
  ...
  private def getOutputParameters(): F[ResultSetImpl[F]] =
    outputParameterResult.get.flatMap {
      ...
      case Some(resultSet) => F.pure(resultSet)
    }
...
object CallableStatementImpl:
  ...
  object ParamInfo:
    ...
    def apply[F[_]](
      ...
      isFunctionCall: Boolean
    )(using Concurrent[F]): F[ParamInfo] =
