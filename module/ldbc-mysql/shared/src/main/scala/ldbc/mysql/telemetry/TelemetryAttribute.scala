 ...
 *
 * The inlined OpenTelemetry semantic-convention attribute keys the driver uses are available via
 * [[DbAttributes]], [[ServerAttributes]], and [[ErrorAttributes]] (see `SemanticAttributes.scala`).
 *
