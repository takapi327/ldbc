...
private[ldbc] trait SharedPreparedStatement extends PreparedStatement[Fx], StatementImpl.ShareStatement:
  ...
  def params: Ref[SortedMap[Int, Parameter]]

  override def setNull(index: Int, sqlType: Int): Fx[Unit] =
    params.update(_ + (index -> Parameter.none))

  override def setBoolean(index: Int, value: Boolean): Fx[Unit] =
    params.update(_ + (index -> Parameter.boolean(value)))

  override def setByte(index: Int, value: Byte): Fx[Unit] =
    params.update(_ + (index -> Parameter.byte(value)))

  override def setShort(index: Int, value: Short): Fx[Unit] =
    params.update(_ + (index -> Parameter.short(value)))

  override def setInt(index: Int, value: Int): Fx[Unit] =
    params.update(_ + (index -> Parameter.int(value)))

  override def setLong(index: Int, value: Long): Fx[Unit] =
    params.update(_ + (index -> Parameter.long(value)))

  override def setFloat(index: Int, value: Float): Fx[Unit] =
    params.update(_ + (index -> Parameter.float(value)))

  override def setDouble(index: Int, value: Double): Fx[Unit] =
    params.update(_ + (index -> Parameter.double(value)))

  override def setBigDecimal(index: Int, value: BigDecimal): Fx[Unit] =
    params.update(_ + (index -> Parameter.bigDecimal(value)))

  override def setString(index: Int, value: String): Fx[Unit] =
    params.update(_ + (index -> Parameter.string(value)))

  override def setBytes(index: Int, value: Array[Byte]): Fx[Unit] =
    params.update(_ + (index -> Parameter.bytes(value)))

  override def setTime(index: Int, value: LocalTime): Fx[Unit] =
    params.update(_ + (index -> Parameter.time(value)))

  override def setDate(index: Int, value: LocalDate): Fx[Unit] =
    params.update(_ + (index -> Parameter.date(value)))

  override def setTimestamp(index: Int, value: LocalDateTime): Fx[Unit] =
    params.update(_ + (index -> Parameter.datetime(value)))

  override def setObject(parameterIndex: Int, value: Object): Fx[Unit] =
    value match
  ...
  override def executeUpdate(): Fx[Int] = executeLargeUpdate().map(_.toInt)

  override def executeBatch(): Fx[Array[Int]] =
