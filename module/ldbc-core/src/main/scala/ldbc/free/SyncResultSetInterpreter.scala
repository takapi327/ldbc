...
def syncResultSetInterpreter(row: SyncRow): ResultSetOp.Visitor[Id] = new ResultSetOp.Visitor[Id]:
  ...
  override def getTimestamp(columnLabel:  String): Id[LocalDateTime] = throw new NonSyncDecoderException("getTimestamp(label)")
  override def getBigDecimal(columnLabel: String): Id[BigDecimal]    = throw new NonSyncDecoderException("getBigDecimal(label)")
