...
private[jdbc] case class ResultSetImpl[F[_]: Sync](resultSet: java.sql.ResultSet) extends ResultSet[F]:
  ...
  override def getDate(columnIndex: Int): F[LocalDate] = Sync[F].blocking(
    resultSet.getDate(columnIndex) match
      case null => null
      case date => date.toLocalDate
  )

  override def getDate(columnLabel: String): F[LocalDate] = Sync[F].blocking(
    resultSet.getDate(columnLabel) match
      case null => null
      case date => date.toLocalDate
  )

  override def getTime(columnIndex: Int): F[LocalTime] = Sync[F].blocking(
    resultSet.getTime(columnIndex) match
      case null => null
      case time => time.toLocalTime
  )

  override def getTime(columnLabel: String): F[LocalTime] = Sync[F].blocking(
    resultSet.getTime(columnLabel) match
      case null => null
      case time => time.toLocalTime
  )

  override def getTimestamp(columnIndex: Int): F[LocalDateTime] = Sync[F].blocking(
    resultSet.getTimestamp(columnIndex) match
      case null      => null
      case timestamp => timestamp.toLocalDateTime
  )

  override def getTimestamp(columnLabel: String): F[LocalDateTime] = Sync[F].blocking(
    resultSet.getTimestamp(columnLabel) match
      case null      => null
      case timestamp => timestamp.toLocalDateTime
  )

  override def getMetaData(): F[ResultSetMetaData] = Sync[F].blocking(
    ResultSetMetaDataImpl(resultSet.getMetaData)
  )

  override def getBigDecimal(columnIndex: Int): F[BigDecimal] = Sync[F].blocking(
    resultSet.getBigDecimal(columnIndex)
  )

  override def getBigDecimal(columnLabel: String): F[BigDecimal] = Sync[F].blocking(
