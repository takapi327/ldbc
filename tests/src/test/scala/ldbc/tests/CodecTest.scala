    ...
    assertIO(
      connection.use { conn =>
        ...
        (for
          ...
          result <- sql"SELECT * FROM array_byte_test WHERE data = ${Array[Byte](1, 2, 3)}".query[Array[Byte]].to[Option]
    ...
    assertIO(
      connection.use { conn =>
        ...
        (for
          ...
          result <- sql"SELECT * FROM local_time_test WHERE time = ${LocalTime.of(12, 34, 56)}".query[LocalTime].to[Option]
        yield result).transaction(conn)
      },
    ...
    assertIO(
      connection.use { conn =>
        ...
        (for
          ...
          result <- sql"SELECT * FROM local_date_test WHERE date = ${LocalDate.of(2023, 4, 5)}".query[LocalDate].to[Option]
        yield result).transaction(conn)
      },
    ...
    assertIO(
      connection.use { conn =>
        ...
        (for
          ...
          _ <- sql"INSERT INTO local_date_time_test (date_time) VALUES (${LocalDateTime.of(2023, 4, 5, 12, 34, 56)})".update
          result <- sql"SELECT * FROM local_date_time_test WHERE date_time = ${LocalDateTime.of(2023, 4, 5, 12, 34, 56)}".query[LocalDateTime].to[Option]
        yield result).transaction(conn)
      },
    ...
    assertIO(
      connection.use { conn =>
        ...
        (for
          ...
          result <- sql"SELECT * FROM year_month_test WHERE ymonth = ${YearMonth.of(2023, 4)}".query[YearMonth].to[Option]
        yield result).transaction(conn)
      },
