...
trait ConnectionFixture:
  ...
  def withBeforeAll(f: Connection[IO] => IO[Unit]): ConnectionFixture

  def withAfterAll(f: Connection[IO] => IO[Unit]): ConnectionFixture

  def withBeforeEach(f: Connection[IO] => IO[Unit]): ConnectionFixture

  def withAfterEach(f: Connection[IO] => IO[Unit]): ConnectionFixture
