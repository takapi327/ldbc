...
class LdbcConnectionPoolDslTest extends ConnectionPoolDslTest:
  ...
  override def config: MySQLConfig = MySQLConfig
    .default
    .setPort(13306)
    .setUser("ldbc")
  ...
  test(s"$prefix: ConnectionPool.use with join query") {
    PooledDataSource
      ...
      (country join city)
        ...
        results.foreach { case (countryName, cityName) =>
  ...
  test(s"$prefix: ConnectionPool.use handles query exceptions properly") {
    PooledDataSource
      ...
      for
        ...
        result <- sql"SELECT * FROM non_existent_table".query[String].to[List].readOnly(Connector.fromDataSource(pool)).attempt
  ...
  test(s"$prefix: ConnectionPool metrics tracking with dsl queries") {
    ...
    for
      ...
      pool <- PooledDataSource
        .fromConfig[IO](config.setMinConnections(1).setMaxConnections(3), None, Some(tracker)).allocated.map(_._1)
