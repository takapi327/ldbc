...
Additionally, if you need to use the low-level Connection API directly, you can obtain a connection with the `use` extension method from `ldbc.mysql.syntax.*`:

```scala 3
import ldbc.mysql.syntax.*

datasource.use { conn =>
  // Use the Connection API directly
}
```
...
In the following examples, we obtain the MySQL-specific `LdbcConnection` (which provides the `Statement`/`PreparedStatement` creation methods) with the `ldbc.mysql.Connection` factory and use it with `use`.
...
```scala 3
import ldbc.mysql.syntax.*  // enable the whileM extension method
...
These utility commands are provided by the MySQL-specific `LdbcConnection`. Because the `ldbc.mysql.Connection` factory returns `Resource[F, LdbcConnection[F]]`, the following examples use this `connection` (`getStatistics`/`changeUser`/`resetServerState` and others are methods of `LdbcConnection`).
