...
```scala
connection.use { conn =>
  val statement: Statement[IO] = conn.createStatement()
  val result: IO[ResultSet] = statement.executeQuery("SELECT * FROM users")
  // ResultSetを使用した処理
}
```
...
connection.use { conn =>
  val statement: Statement[IO] = conn.createStatement()
  val result: IO[Int] = statement.executeUpdate("INSERT INTO users (name, age) VALUES ('Alice', 20)")
}
```
...
connection.use { conn =>
  val statement: Statement[IO] = conn.createStatement()
  val result: IO[Int] = statement.returningAutoGeneratedKey("INSERT INTO users (name, age) VALUES ('Alice', 20)")
}
```
