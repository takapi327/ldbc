...
private[ldbc] trait Table[F[_], P <: Product] extends Dynamic:
  ...
  def selectDynamic[Tag <: Singleton](tag: Tag)(
    using mirror: Mirror.ProductOf[P], index: ValueOf[Tuples.IndexOf[mirror.MirroredElemLabels, Tag]]
...
object Table extends Dynamic:
   ...
   *   A class that implements a [[Product]] that is one-to-one with the table definition.
   */
  def applyDynamic[F[_], P <: Product](nameApply: "apply")(
    using mirror: Mirror.ProductOf[P], converter: ColumnTupleConverter[mirror.MirroredElemTypes, F]
  )(name: String)(columns: ColumnTuples[mirror.MirroredElemTypes, F]): Table[F, P] =
   ...
   *   A class that implements a [[Product]] that is one-to-one with the table definition.
   */
  private def fromTupleMap[F[_], P <: Product](using mirror: Mirror.ProductOf[P])(
    _name:   String,
    ...
    override def selectDynamic[Tag <: Singleton](tag: Tag)(
      using mirror: Mirror.ProductOf[P], index: ValueOf[Tuples.IndexOf[mirror.MirroredElemLabels, Tag]]
    ): Column[F, Tuple.Elem[mirror.MirroredElemTypes, Tuples.IndexOf[mirror.MirroredElemLabels, Tag]]] =
      columns.productElement(index.value)
