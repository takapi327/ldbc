...
object ColumnTupleConverter:
  ...
  given [T1, T2, TN <: NonEmptyTuple, F[_]](using converter: ColumnTupleConverter[T2 *: TN, F]): ColumnTupleConverter[T1 *: T2 *: TN, F] =
  ...
  /** Method for converting Column Tuple to Tuple Map.
    ...
    *   The effect type
    */
  def convert[Types <: Tuple, F[_]](columnTuples: ColumnTuples[Types, F])(using converter: ColumnTupleConverter[Types, F]): Tuple.Map[Types, F] =
