 ...
 * It will be unnecessary if enums can be identified by type parameters.
 *
 * example:
 * {{{
...
/**
 * Trait inherited to access the standard implementation that the generated enum has.
 * It will be unnecessary if enums can be identified by type parameters.
 *
 * @tparam T
 *   Trait for using enum with Ldbc.
...
trait EnumDataType[T <: Enum]:
  ...
  /**
   * Method to obtain the Enum value at the specified index number,
   * a value that is standard implemented when the enum is generated.
   *
   * @param ordinal
  ...
  /**
   * Method to retrieve the Enum value with the specified name,
   * a value that is standard implemented when the enum is generated.
   *
   * @param name
   *   enum name
